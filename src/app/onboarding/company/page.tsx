        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} required
              ...
              placeholder="CoolAir Mechanical" />
          </div>
          <div>
cat > src/app/onboarding/territory/page.tsx << 'EOF'
"use client";
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg">
          ...
          {selectedZips.length > 0 && (
            <div className="flex gap-2 flex-wrap">
              {selectedZips.map((zip) => (
                <span key={zip} onClick={() => toggleZip(zip)}
                  className="px-3 py-1 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 text-xs font-medium cursor-pointer hover:bg-green-500/20 transition-colors">
                  {zip} ✕
                </span>
              ))}
            </div>
        ...
        <div className="space-y-2 max-h-96 overflow-y-auto mb-6">
          {Object.entries(FLORIDA_ZIPS).map(([county, zips]) => (
            <div key={county} className="border border-zinc-800 rounded-lg overflow-hidden">
              <button onClick={() => setExpandedCounty(expandedCounty === county ? null : county)}
                className="w-full px-4 py-3 flex items-center justify-between text-left bg-zinc-900 hover:bg-zinc-800 transition-colors">
              ...
              {expandedCounty === county && (
                <div className="px-4 py-3 bg-zinc-950 flex flex-wrap gap-2">
                  {zips.map((zip) => {
                    ...
                    return (
                      <button key={zip} onClick={() => toggleZip(zip)} disabled={isDisabled}
                        className={`px-3 py-1.5 rounded-md text-xs font-mono transition-colors ${
                          isSelected ? "bg-green-500/20 border border-green-500/30 text-green-400"
                          : isDisabled ? "bg-zinc-900 border border-zinc-800 text-zinc-600 cursor-not-allowed"
                          : "bg-zinc-900 border border-zinc-800 text-zinc-300 hover:border-zinc-600 cursor-pointer"
                        }`}>
                        {zip}
                      </button>
                    );
        ...
        <button onClick={handleSubmit} disabled={loading || selectedZips.length === 0}
          ...
          {loading ? "Setting up your dashboard..." : `Continue with ${selectedZips.length} zip${selectedZips.length !== 1 ? "s" : ""}`}
        </button>
      </div>
    </div>
  );
