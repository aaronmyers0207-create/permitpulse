...
const CATEGORY_LABELS: Record<string, string> = {
  hvac_changeout: "HVAC Change-out", hvac_new_install: "New HVAC Install",
  new_construction: "New Construction", renovation_mechanical: "Renovation",
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {filteredPermits.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-zinc-500 text-sm max-w-md mx-auto">We are pulling permits for your territory. New data comes in twice daily. Check back soon.</p>
          </div>
        ) : (
              ...
              <tbody>
                {filteredPermits.map((permit: any) => (
                  <tr key={permit.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/50 transition-colors">
                    ...
                    <td className="px-4 py-3"><span className={"inline-block px-2.5 py-1 rounded-full text-xs font-medium border " + (CATEGORY_COLORS[permit.category] || CATEGORY_COLORS.general_mechanical)}>{CATEGORY_LABELS[permit.category] || permit.permit_type || "Permit"}</span></td>
